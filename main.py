...
There is no difference between an NRI PAN card and normal PAN card. An [NRI PAN card is the same as a PAN card issued to individuals living in India](https://ABC.com/blog/nri-bank-accounts/what-is-the-difference-between-an-nri-pan-card-and-a-normal-pan-card#:~:text=There%20is%20only%20one%20Pan,to%20Indian%20Residents%20and%20NRIs.).

However, to apply for a PAN card for OCI or people of Indian origin who hold foreign citizenship, an applicant is required to fill Form 49AA.
...
During the PAN card correction process, you will be either provided with a payment link or taken to Razorpay page. Razorpay is a secure online platform where you can make the payment conveniently using various payment options such as credit/debit cards, net banking, or digital wallets. Make sure to follow the instructions provided and ensure the payment is made within''',
...
Yes, you can take the delivery of your PAN card only at an Indian address mentioned in your Aadhaar card. While applying for a new PAN card or requesting corrections, you can provide your Indian address as the delivery address. Make sure to provide accurate and complete address details to ensure successful delivery.'''
  ...
  if st.button("Submit"):
    ...
    # matching_docs = vector_store.similarity_search(user_query)

    # Use the retrieval_chain to get the answer from the relevant documents
    # answer = retrieval_chain.run(query=user_query, input_documents=matching_docs)

    # Process the answer and display it

    print(len(new_list))
    # for i in range(len(new_list)):
    # filter(None, answer.split('\n')))
    if re.findall(user_query, new_list[0]):
      # print(new_list[0])
      ...
      exit()
      # f"{i}. {item}")
    elif re.findall(user_query, new_list[1]):
      # print(new_list[1])
      ...
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[2]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[2])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[3]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[3])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[4]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[4])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[5]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[5])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[6]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[6])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[7]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[7])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[8]):
      ...
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[9]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[9])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[10]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[10])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[11]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[11])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[12]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[12])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[13]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[13])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[14]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[14])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[15]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[15])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[16]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[16])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[17]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[17])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[18]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[18])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[19]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[19])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[20]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[20])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[21]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[21])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[22]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[22])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[23]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[23])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[24]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[24])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[25]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[25])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[26]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[26])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[27]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[27])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[28]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[28])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[29]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[29])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[30]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[30])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[31]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[31])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[32]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[32])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[33]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[33])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[34]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[34])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[35]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[35])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[36]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[36])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[37]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[37])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[38]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[38])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[39]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[39])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[40]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[40])
    elif re.findall(user_query, new_list[41]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[41])
      # f"{i}. {item}")
      exit()
    elif re.findall(user_query, new_list[42]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[42])
      # f"{i}. {item}")
      exit()
      # f"{i}. {item}")
    elif re.findall(user_query, new_list[43]):

      # for i, item in enumerate(new_list, start=1):
      st.write(new_list[43])
      # f"{i}. {item}")
      exit()

    else:
      st.write("Sorry, I couldn't find an answer to your query.")
